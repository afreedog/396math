...
\textbf{三、指数函数}
\section*{3．指数函数}
$y=a^2(a>0, a \neq 1)$ ，其定义域为 $(-\infty,+\infty)$ 。当 $0<a<1$ 时，函数严格单调递减。当 $a>1$ 时，函数严格单调递增．指数函数的图形恒过点（0 ，1）。微积分中经常用到以 e为底的指数函数，即 $y=\mathrm{e}^x$ 。指数函数 $y=a^x$ 的性质归纳见下表。



\begin{tabular}[t]{|c|c|c|}
	\hline & $a>1$ & $0<a<1$ \\
	\hline 图像 &  \centering \resizebox{4cm}{!}{
		\begin{tikzpicture}[>=stealth, font=\large]
			...
			\draw[->, thick, gray!80!black] (0,-0.5) -- (0,5.2) node[left=1pt] {\(y\)};
			\node[below left=1pt, inner sep=1pt] at (0,0) {\(O\)};
			\draw[skyblue, very thick, domain=-1.8:2.2, samples=100] plot (\x, {2^\x});
		\end{tikzpicture}
	}  & \\
	\hline \multirow{5}{*}{性质} & \multicolumn{2}{|l|}{（1）定义域： $\mathbf{R}$} \\
	\hline & \multicolumn{2}{|l|}{（2）值域：$(0,+\infty)$ ，图像在 $x$ 轴上方} \\
	\hline & （3）过点（0，1），即 $x=0$ 时，$y=1$ & \\
	\hline & （4）当 $a>1$ 时，$\left\{\begin{array}{l}\text { 若 } x>0, \text { 则 } a^x>1 \\ \text { 若 } x<0, \text { 则 } 0<a^x<1\end{array}\right.$ & 当 $0<a<1$ 时，$\left\{\begin{array}{l}\text { 若 } x<0, \text { 则 } a^x>1 \\ \text { 若 } x>0, \text { 则 } 0<a^x<1\end{array}\right.$ \\
	\hline & （5）在 $\mathbf{R}$ 上是增函数 & 在 $\mathbf{R}$ 上是减函数 \\
	\hline
\end{tabular}
