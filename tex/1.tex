...
以下四个函数	
\begin{center}
	\ding{172}  $f_1(x)=\frac{\mathrm{e}^x+\mathrm{e}^{-1}}{2}$ \qquad \qquad 	\ding{173}  $\int_2(x)=\frac{\mathrm{e}^x-\mathrm{e}^{-x}}{2}$\\
	\ding{174} $\int_3(x)=\ln \frac{1-x}{1+x}$ \qquad \qquad \ding{175} $\int_4(x)=\ln \left(x+\sqrt{x^2+1}\right)$
\end{center}
